...
def create_screen(screen_name: str) -> None:
    ...
    try:
        create_file_or_directory(destination_path, PRESENTATION_TEMPLATE_PATH, screen_name, root_code_path)
        create_file_or_directory(destination_path, UI_TEMPLATE_PATH, screen_name, root_code_path)
